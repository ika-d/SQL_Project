...
#6 
select dayname(order_date) as day_of_week, count(distinct order_id) as total_orders_per_day
from pizza_data 
group by day_of_week;

#7 
select hour(order_time) as order_hour, count(distinct order_id) as total_orders_per_hour
from pizza_data
group by order_hour
order by order_hour;
